def truncate_text(s):
    ...
    for pos in range(len(s)):
        ...
        #print(f'Posic:{pos}-{s[pos]}>{values_dic[s[pos]]} , Total: {total}')
        if total > 50:
            if (total-values_dic[s[pos]]) <= 47:
                #print(f'Posic:{pos}-{s[pos]} , Total: {total}')
                #print(total-values_dic[s[pos]]+3)
                return s[:pos]+'...'
            elif (total-values_dic[s[pos]]-values_dic[s[pos-1]]) <= 47:
                #print(f'Posic:{pos}-{s[pos]} , Total: {total}')
                #print(total-values_dic[s[pos]]-values_dic[s[pos-1]]+3)
                return s[:pos-1]+'...'
            elif (total-values_dic[s[pos]]-values_dic[s[pos-1]]-values_dic[s[pos-2]]) <= 47:
                #print(f'Posic:{pos}-{s[pos]} , Total: {total}')
                #print(total-values_dic[s[pos]]-values_dic[s[pos-1]]-values_dic[s[pos-2]]+3)
                return s[:pos-2]+'...'
